...
public class AgenceServiceImpl implements AgenceService {
    ...
    public long addAgence(AgenceRequest agenceRequest) {
        ...
        Agence agence;
        if(agenceRepository.existsBySlug(Str.slug(agenceRequest.getNom()))){
            agence = agenceRepository.findBySlug(Str.slug(agenceRequest.getNom())).orElseThrow();
        }else{
    ...
    public void addAgence(List<AgenceRequest> agenceRequests) {
        ...
        for (AgenceRequest agenceRequest: agenceRequests) {
            ...
            if(agenceRepository.existsBySlug(Str.slug(agenceRequest.getNom()))){
                agence = agenceRepository.findBySlug(Str.slug(agenceRequest.getNom())).orElseThrow();
                editAgence(agenceRequest, agence.getId());
            }else{
                agence = Agence.builder()
                        .nom(agenceRequest.getNom())
                        .slug(Str.slug(agenceRequest.getNom()))
    ...
    public AgenceResponse getAgenceById(long agenceId) {
        ...
        Agence agence
                ...
                .orElseThrow(
                        () -> new BMCustomException("Agence with given Id not found", NOT_FOUND));
    ...
    @Override
    public List<Agence> getAgenceByQueryString(String q) {
        log.info("AgenceServiceImpl | getAgenceByQueryString is called");
        return agenceRepository.findAllByQueryString(q);
    }
}
