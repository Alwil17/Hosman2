...
import com.dopediatrie.hosman.bm.repository.PosologieRepository;
...
public class PosologieServiceImpl implements PosologieService {
    ...
    public long addPosologie(PosologieRequest posologieRequest) {
        ...
        Posologie posologie;
        if(posologieRepository.existsBySlug(Str.slug(posologieRequest.getLibelle()))){
            posologie = posologieRepository.findBySlug(Str.slug(posologieRequest.getLibelle())).orElseThrow();
            editPosologie(posologieRequest, posologie.getId());
        }else{
            posologie = Posologie.builder()
    ...
    public PosologieResponse getPosologieById(long posologieId) {
        ...
        Posologie posologie
                ...
                .orElseThrow(
                        () -> new BMCustomException("Posologie with given Id not found", NOT_FOUND));
    ...
    @Override
    public List<Posologie> getPosologiesByTypeAndQueryString(String type, String q) {
        log.info("PosologieServiceImpl | getPosologiesByTypeAndQueryString is called");
        return posologieRepository.findByTypeAndQueryString(type, q);
    }

    @Override
    public List<Posologie> getPosologiesByType(String type) {
        log.info("PosologieServiceImpl | getPosologiesByType is called");
        return posologieRepository.findByType(type);
    }
}
